...
function RegisterPageInner() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			// Redirect to application if period is provided
			if (period) {
		...
		<div className="min-h-screen flex items-center justify-center p-4">
			...
			<Card className="relative w-full max-w-md animate-scale-in">
				<CardHeader className="text-center">
					...
					<h1 className="text-2xl font-display font-bold text-surface-900">Regisztráció</h1>
					<p className="text-surface-500 mt-2">Hozd létre a fiókodat</p>
					{period && (
						<p className="text-sm text-primary-600 mt-2 font-medium">
							Jelentkezés időszak: {period}
						</p>
					)}
				</CardHeader>
				...
				<form onSubmit={handleSubmit}>
					...
					<CardFooter className="flex flex-col gap-4">
						...
						<p className="text-sm text-center text-surface-500">
							Már van fiókod?{' '}
							<Link
								href={period ? `/login?period=${period}` : '/login'}
